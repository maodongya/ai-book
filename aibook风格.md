...
首版内置 **8 套预设**。金色强调按钮**仅**「古典书房」使用；其余主题用各自装帧色（墨、陶土、竹青、朱砂、绛紫、石青、系统蓝）。
...
| 主题 ID | 名称 | 模式 | 按钮强调色 | 适合场景 |
|---------|------|------|------------|----------|
| `classicStudy` | 古典书房 | 浅 | **金色**（仅此套） | 默认、沉浸阅读 |
| `plainInk` | 素笺墨香 | 浅 | 墨色 | 长文专注 |
| `nightLamp` | 深夜灯盏 | 深 | 陶土 | 夜间暖光 |
| `bambooScroll` | 青简竹简 | 浅 | 竹青 | 文艺、古籍 |
| `cinnabarHall` | 朱砂厅堂 | 浅 | 朱砂 | 喜庆、线装 |
| `plumBlossom` | 梅笺春讯 | 浅 | 绛紫 | 笺纸、春日 |
| `indigoNight` | 靛蓝夜读 | 深 | 石青 | 夜间冷光 |
| `modernClean` | 现代简约 | 浅 | 系统蓝 | 效率、开发 |
